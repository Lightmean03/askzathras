...
class Pipeline:
    ...
    @property
    def pipelines(self):
        """Return pipeline configuration for OpenWebUI - this must be a property"""
        return [
            {
                "id": self.id,
                "name": self.name
        ...
        try:
            ...
            if not self.initialized:
                if not self._load_rag_system():
                    return "Error: RAG system could not be initialized. Please check your FAISS index path and Ollama model availability."
...
# Test the pipeline when imported
if __name__ == "__main__":
    print("Testing pipeline...")
    try:
        p = Pipeline()
        print(f"Pipeline created successfully: {p.id}")
        print(f"Has pipelines property: {hasattr(p, 'pipelines')}")
        print(f"Pipelines: {p.pipelines}")
        print(f"Can call pipelines: {callable(p.pipelines)}")
    except Exception as e:
